...
const Mode = ({model}) => {
    return (
        <div className='border border-zinc-200 rounded-xl overflow-hidden shadow-lg'>
           ...
           <img className='w-40 h-40 object-contain transition-transform duration-700 hover:scale-125' src={model.image} alt={model.image} />
            <p className='absolute top-0 -right-14 inline-flex border border-red-500 px-2 rounded-full bg-red-400 text-white'>{model.status}</p>
           </div>
            </div>
